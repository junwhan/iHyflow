...
import org.infinispan.iHyflow.benchmark.Benchmark;
...
import org.infinispan.iHyflow.helper.AbstractLock;
							...
							// ControlFlowDirecotry
{
	...
	public static long totalBalance(final String accountNum1,
			final String accountNum2) {
		Long res = null;
		try {
			res = new Atomic<Long>() {
				@Override
				public Long atomically(AbstractDistinguishable self,
					...
					try {
						...
						for (int i = 0; i < Benchmark.calls; i++)
							balance += BankAccount.checkBalance(accountNum1);

						try {
							for (int i = 0; i < Benchmark.calls; i++)
								balance += BankAccount
										.checkBalance(accountNum2);
						} catch (TransactionException e) {
							throw e;
						} catch (Throwable e) {
							e.printStackTrace(Logger.levelStream[Logger.DEBUG]);
						...
						org.infinispan.iHyflow.benchmark.Benchmark.processingDelay();
					}
				}
			}.execute(null);
		} catch (TransactionException e) {
	...
	public static void transfer(final String accountNum1,
			final String accountNum2, final int amount) {
		try {
			new Atomic<Object>() {
				@Override
				public Object atomically(AbstractDistinguishable self,
					...
					try {
						...
						for (int i = 0; i < Benchmark.calls; i++)
							BankAccount.withdraw(accountNum1, amount);

						try {
							for (int i = 0; i < Benchmark.calls; i++)
								BankAccount.deposit(accountNum2, amount);
						} catch (TransactionException e) {
							throw e;
						} catch (Throwable e) {
							e.printStackTrace(Logger.levelStream[Logger.DEBUG]);
