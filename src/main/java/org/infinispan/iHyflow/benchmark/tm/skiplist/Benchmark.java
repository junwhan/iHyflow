package org.infinispan.iHyflow.benchmark.tm.skiplist;
...
public class Benchmark extends org.infinispan.iHyflow.benchmark.tm.Benchmark
{
	...
	protected void createLocalObjects() {
		...
		if (nodes >= 3) {
			SkipList<Integer> sl = new SkipList<Integer>("skiplist-"+id);
			for (int j=0; j<40; j++) {
				sl.insert(random.nextInt(MAX_KEY));
			}
		} else {
			for (int i=0; i<3; i++) {
				if ((i % nodes) == id) {
					SkipList<Integer> sl = new SkipList<Integer>("skiplist-"+i);
					for (int j=0; j<40; j++) {
						sl.insert(random.nextInt(MAX_KEY));
					}
				}
	...
	protected void readOperation(Object... ids) {
		...
		for (int i=0; i<ids.length; i++)
			i2[i] = (skipid)ids[i];
		if (r == -1) { 
			//talex: operation too long, very slim chances of commit! 
			//TODO: talex: requires support, such as early locks
