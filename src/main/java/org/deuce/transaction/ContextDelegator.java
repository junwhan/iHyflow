...
import org.infinispan.iHyflow.core.IDistinguishable;
import org.infinispan.iHyflow.core.tm.NestedContext;
import org.infinispan.iHyflow.core.tm.NestingModel;
import org.infinispan.iHyflow.core.tm.ContextFactory;
...
public class ContextDelegator {
	...
	final private static ContextFactoryThreadLocal THREAD_CONTEXT_FACTORY = new ContextFactoryThreadLocal();
	
	public static ContextFactory getFactory()
	{
		return THREAD_CONTEXT_FACTORY.get();
	}
	
	// talex: changed this to a ThreadLocal<ContextFactory> (from ThreadLocal<AbstractContext>)
	// talex: moved remaining logic into ContextFactory
	@Exclude
	private static class ContextFactoryThreadLocal extends ThreadLocal<ContextFactory>
	{
		@Override
		protected synchronized ContextFactory initialValue() {
			try {
	...
	public static void setNestingModel(NestingModel model) {
		// dispatch this to the appropriate ContextFactory
		THREAD_CONTEXT_FACTORY.get().setNestingModel(model);
	}

	public static AbstractContext getInstance(){
		// dispatch this to the appropriate ContextFactory
		return THREAD_CONTEXT_FACTORY.get().getInstance();
	}
	
	public static AbstractContext getTopInstance() {
		// displatch it to ContextFactory
		return THREAD_CONTEXT_FACTORY.get().getTopInstance();
	}
	
	// talex: Added a method to remove one level off the context stack
	public static void releaseInstance(Context context){
		// dispatch this to the appropriate ContextFactory
		THREAD_CONTEXT_FACTORY.get().releaseInstance(context);
	}
	...
	public static void onOpenObject(Object object) {
		final AbstractContext context = THREAD_CONTEXT_FACTORY.get().getTopInstance();
		if (context != null && context instanceof NestedContext && object instanceof IDistinguishable) {
			((NestedContext)context).onOpenObject((AbstractDistinguishable)object);
		}
	}
